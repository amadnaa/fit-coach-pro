...
export default function ClientDashboard() {
  ...
  const firstName = user?.user_metadata?.full_name?.split(' ')[0] || t('dashboard.athlete');
  ...
  return (
    <MobileLayout>
      {/* Header Banner */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card border-b border-border px-5 pt-6 pb-5">
      ...
      <div className="px-5 pt-5 space-y-5 pb-4">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="p-5 rounded-2xl gradient-primary text-primary-foreground space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-semibold text-lg">{t('dashboard.todaysWorkout')}</p>
                {loadingTodayWorkout ? (
                  <p className="text-sm opacity-80">{t('dashboard.loading')}</p>
                ) : todayWorkout ? (
                  <p className="text-sm opacity-80">{todayWorkout.workoutName} · {todayWorkout.exerciseCount} {t('dashboard.exercises')}</p>
                ) : (
                  <p className="text-sm opacity-80">{t('dashboard.noWorkoutScheduled')}</p>
                )}
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="space-y-3">
          <h2 className="text-lg font-display font-semibold">{t('dashboard.warmupStretching')}</h2>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3 rounded-2xl bg-card border border-border space-y-2 cursor-pointer hover:border-primary/30 transition-colors" onClick={() => setWeightDialogOpen(true)}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1">
                ...
                <p className="text-[11px] font-medium">{t('dashboard.bodyweight')}</p>
              </div>
            ...
            {bodyweightData.length >= 1 ? (
              ...
              <div className="h-20 flex items-center justify-center">
                <p className="text-[10px] text-muted-foreground">{t('dashboard.tapToLogWeight')}</p>
              </div>
            )}
          </div>
          ...
          <div className="p-3 rounded-2xl bg-card border border-border space-y-2 cursor-pointer hover:border-primary/30 transition-colors" onClick={() => setSleepDialogOpen(true)}>
            <div className="flex items-center gap-1">
              ...
              <p className="text-[11px] font-medium">{t('dashboard.sleep')}</p>
            </div>
            {sleepData.length > 1 ? (
              <ResponsiveContainer width="100%" height={80}>
              ...
              <div className="h-20 flex items-center justify-center">
                <p className="text-[10px] text-muted-foreground">{t('dashboard.tapToLogSleep')}</p>
              </div>
            )}
          </div>
          )}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="space-y-3">
          <h2 className="text-lg font-display font-semibold">{t('dashboard.weeklyCheckin')}</h2>
          {checkInSubmitted ? (
            <div className="p-4 rounded-2xl bg-card border border-border text-center space-y-1">
              <p className="text-sm font-medium text-primary">{t('dashboard.submittedThisWeek')}</p>
              <p className="text-xs text-muted-foreground">{t('dashboard.coachWillReview')}</p>
            </div>
          ) : (
            <div className="p-4 rounded-2xl bg-card border border-border space-y-4">
              {/* Training Difficulty */}
              <div className="space-y-2">
                <p className="text-xs font-medium">{t('dashboard.trainingHardQ')}</p>
                <div className="flex gap-2">
                  {(['easy', 'moderate', 'hard', 'very_hard'] as const).map(v => (
                    <button key={v} onClick={() => setWeeklyCheckIn(p => ({ ...p, training_difficulty: v }))}
                      className={cn("flex-1 py-2 rounded-xl text-[10px] font-medium border transition-colors",
                      ...
                      )}>{t(`dashboard.diff.${v}`)}</button>
                  ))}
                </div>
              </div>
              {/* Recovery */}
              <div className="space-y-2">
                <p className="text-xs font-medium">{t('dashboard.recoveryQ')}</p>
                <div className="flex gap-2">
                  {(['poor', 'okay', 'good', 'great'] as const).map(v => (
                    <button key={v} onClick={() => setWeeklyCheckIn(p => ({ ...p, recovery_level: v }))}
                      ...
                      )}>{t(`dashboard.rec.${v}`)}</button>
                  ))}
                </div>
              </div>
              {/* Energy */}
              <div className="space-y-2">
                <p className="text-xs font-medium">{t('dashboard.energyQ')}</p>
                <div className="flex gap-2">
                  {(['low', 'moderate', 'high', 'peak'] as const).map(v => (
              ...
              >
                {submitting ? t('dashboard.submitting') : t('dashboard.submitCheckin')}
              </button>
            </div>
          )}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="space-y-3">
          <h2 className="text-lg font-display font-semibold">{t('dashboard.quickStats')}</h2>
          <div className="grid grid-cols-2 gap-3">
            {[
              { icon: Dumbbell, label: t('dashboard.workouts'), value: String(workoutCount), sublabel: t('dashboard.thisMonth') },
              { icon: Weight, label: t('dashboard.volume'), value: totalVolume >= 1000 ? `${(totalVolume / 1000).toFixed(1)}t` : `${totalVolume}kg`, sublabel: t('dashboard.liftedThisMonth') },
            ].map((stat) => (
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="space-y-3">
          <h2 className="text-lg font-display font-semibold">{t('dashboard.recentActivity')}</h2>
          {recentSessions.length > 0 ? (
            recentSessions.map((session) => (
              <div key={session.id} className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium">{t('dashboard.workout')}</p>
                ...
                <div className="text-right">
                  <span className="text-xs text-primary font-medium">{t('dashboard.completed')}</span>
                  {session.exercises_completed != null && session.total_exercises != null && (
                    <p className="text-[10px] text-muted-foreground">{t('dashboard.exercisesCountFrac', { a: session.exercises_completed, b: session.total_exercises })}</p>
                  )}
                </div>
              </div>
            ))
            ...
            <div className="p-4 rounded-2xl bg-card border border-border text-center">
              <p className="text-sm text-muted-foreground">{t('dashboard.noCompletedWorkouts')}</p>
              <p className="text-xs text-muted-foreground mt-1">{t('dashboard.completeOneToSee')}</p>
            </div>
          )}
        </motion.div>
      </div>
      ...
      <Dialog open={weightDialogOpen} onOpenChange={setWeightDialogOpen}>
        <DialogContent className="max-w-xs mx-auto">
          <DialogHeader>
            <DialogTitle>{t('dashboard.logBodyweight')}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {bodyweightData.length > 0 && (
              <p className="text-sm text-muted-foreground">
                {t('dashboard.last')} <span className="font-medium text-foreground">{bodyweightData[bodyweightData.length - 1].weight} kg</span>
              </p>
            )}
            <div className="space-y-2">
              <label className="text-sm font-medium">{t('dashboard.weightKgLabel')}</label>
              <Input
                type="number"
                step="0.1"
            ...
            <Button onClick={handleLogWeight} disabled={savingWeight || !newWeight} className="w-full gradient-primary text-primary-foreground">
              {savingWeight ? t('common.saving') : t('dashboard.logWeight')}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={sleepDialogOpen} onOpenChange={setSleepDialogOpen}>
        <DialogContent className="max-w-xs mx-auto">
          <DialogHeader>
            <DialogTitle>{t('dashboard.logSleep')}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {sleepData.length > 0 && (
              <p className="text-sm text-muted-foreground">
                {t('dashboard.last')} <span className="font-medium text-foreground">{sleepData[sleepData.length - 1].hours}h</span>
              </p>
            )}
            <div className="space-y-2">
              <label className="text-sm font-medium">{t('dashboard.hoursSlept')}</label>
              <Input
                type="number"
                step="0.5"
                min="0"
            ...
            <Button onClick={handleLogSleep} disabled={savingSleep || !newSleepHours} className="w-full gradient-primary text-primary-foreground">
              {savingSleep ? t('common.saving') : t('dashboard.logSleep')}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
          ...
          <div className="w-full aspect-video bg-black">
            {selectedVideo?.video_url ? (
              ...
              <div className="w-full h-full flex items-center justify-center">
                <p className="text-sm text-muted-foreground">{t('dashboard.noVideoAvailable')}</p>
              </div>
            )}
          </div>
        </DialogContent>
